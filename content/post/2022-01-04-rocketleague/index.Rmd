...
build_df <- function(lambda) {
  ...
  return(grid%>% tidyr::pivot_wider(names_from = Diff, values_from = Prob))  
}
...
library(gridExtra)
plot_list <- lapply( rl_long$goals_per_minute,  function(x) gridExtra::tableGrob(build_df(x), rows = NULL))
